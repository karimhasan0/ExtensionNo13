function parseAliExpress() {
  ...
  // Remove commas (1,299 → 1299)
  priceText = priceText.replace(/,/g, "");

  // Extract ALL numbers on the price block
  const numbers =
    priceText
      .match(/\d+(\.\d+)?/g)
      ?.map(Number)
  ...
  // AliExpress often shows multiple prices → lowest is the real one
